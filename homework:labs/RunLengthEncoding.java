...
public class RunLengthEncoding implements Iterable {
  ...
  public RunIterator iterator() {
    // Replace the following line with your solution.
	RunIterator firstrun = new RunIterator (this.run.head);
    return firstrun;
  ...
  public PixImage toPixImage() {
	    // Replace the following line with your solution.
		PixImage res = new PixImage (this.width, this.height);
		RunIterator check = this.iterator();
		int i = 0;
		int j = 0;
		int rep = 0;
		while(check.hasNext()){
			...
			res.setPixel(i, j, check.next() [1], check.next() [2], check.next() [3]);
			rep++;
			i++;
			if (rep > check.next() [0]){
				rep = 0;
				check.cur = check.cur.next;
			}
		}
	    return res;
	  }
